...
import { PostType, IPostState } from '/data/posts/reducers';
...
const Post: FC<IProps> = ({ post }) => {
  ...
  // const posts: IPostState = useSelector(selectors.posts.getPosts);
  // if (posts.entities === null) return null;
  // const post: PostType = posts.entities[postSeq];
  const {
    seq,
    createAt,
  ...
  const likeHandler = async (e: SyntheticEvent<HTMLButtonElement>) => {
    e.preventDefault();
    if (!user) return;
    await dispatch(actions.posts.thunkLikePost(1, seq));
